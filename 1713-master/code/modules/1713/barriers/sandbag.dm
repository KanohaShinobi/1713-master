				...
				/* it takes 6 sandbags to make a full sandbag.
				  * so:
				    ...
				    * give people the amount of sandbags (-1 or -0) back
				     ...
				     * place
				*/
				if (!istype(src, /obj/structure/window/sandbag/sandbag/incomplete))
					for (var/v in TRUE to rand(4,6))
						new /obj/item/weapon/sandbag(turf)
				else
					var/obj/structure/window/sandbag/sandbag/incomplete/I = src
					for (var/v in TRUE to (1 + pick(I.progress-1, I.progress)))
						new /obj/item/weapon/sandbag(turf)
				qdel(src)
/*
...
*/
/obj/structure/window/sandbag/sandbag/ex_act(severity)
	switch(severity)
		if (1.0)
			qdel(src)
			return
...
/obj/structure/window/sandbag/sandbag/New(location, var/mob/creator)
	loc = location
	flags |= ON_BORDER
...
/obj/structure/window/sandbag/sandbag/incomplete/ex_act(severity)
	qdel(src)
...
/obj/structure/window/sandbag/sandbag/set_dir(direction)
	dir = direction

// sandbag window overrides

/obj/structure/window/sandbag/sandbag/attackby(obj/O as obj, mob/user as mob)
	return FALSE

/obj/structure/window/sandbag/sandbag/examine(mob/user)
	user << "That's a sandbag."
	return TRUE

/obj/structure/window/sandbag/sandbag/take_damage(var/damage = FALSE, var/sound_effect = TRUE)
	return FALSE

/obj/structure/window/sandbag/sandbag/apply_silicate(var/amount)
	return FALSE

/obj/structure/window/sandbag/sandbag/updateSilicate()
	return FALSE

/obj/structure/window/sandbag/sandbag/shatter(var/display_message = TRUE)
	return FALSE

/obj/structure/window/sandbag/sandbag/bullet_act(var/obj/item/projectile/Proj)
	return FALSE
...
/obj/structure/window/sandbag/sandbag/is_full_window()
	return FALSE

/obj/structure/window/hitby(AM as mob|obj)
	return FALSE // don't move

/obj/structure/window/sandbag/sandbag/attack_generic(var/mob/user, var/damage)
	return FALSE

/obj/structure/window/sandbag/sandbag/rotate()
	return

/obj/structure/window/sandbag/sandbag/revrotate()
	return

/obj/structure/window/sandbag/sandbag/is_fulltile()
	return FALSE
...
//merges adjacent full-tile windows into one (blatant ripoff from game/smoothwall.dm)
/obj/structure/window/sandbag/sandbag/update_icon()
	return

/obj/structure/window/sandbag/sandbag/fire_act(temperature)
	return

/obj/item/weapon/sandbag/sandbag
	name = "sandbag"
	icon_state = "sandbag"
	icon = 'icons/obj/items.dmi'
	w_class = TRUE